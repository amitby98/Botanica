...
//   svG.append("text").attr("class", "y label").attr("text-anchor", "end").attr("y", 6).attr("dy", ".75em").attr("transform", "rotate(-90)").text("average order per item (dollars)");

//   // Graph 2: User average order values
//   var userData = orders.map(order => {
//     const orderQuantity = order.items.reduce((sum, item) => sum + item.quantity, 0);
//     return {
//       x: orderQuantity,
//       y: order.total / order.userCount, // Assuming userCount property exists in order object
//     };
//   });
...
//   userSvG.append("g").call(d3.axisLeft(y));
...
//   userSvG.append("text").attr("class", "y label").attr("text-anchor", "end").attr("y", 6).attr("dy", ".75em").attr("transform", "rotate(-90)").text("average order per user (dollars)");
// }
